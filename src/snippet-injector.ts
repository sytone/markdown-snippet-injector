  ...
  */
	public injectSnippets(docsRoot: string) {
		if (this.storedSnippets.size > 0) {
			for (const [snippet, value] of this.storedSnippets) {
				const {file, header, footer, processedValue} = value;
				if (file) {
					const filePath = `${docsRoot}/${file}`;
					this.ensureDirectoryExistence(filePath);
					const fileContents = `${header ? fs.readFileSync(`${docsRoot}/${header}`, 'utf8') : ''}${processedValue ?? ''}${footer ? fs.readFileSync(`${docsRoot}/${footer}`, 'utf8') : ''}`;
